...
<div class="p-6">
    ...
    if(isset($_POST['btnAdd'])){
        ...
        if($nameuser == "" || $passname == "") {
            $statusMsg = "User name and password cannot be empty.";
            $msgClass = "border-red-500 bg-red-50 text-red-700";
        } else {
            $nameuserEsc = $dbcon->real_escape_string($nameuser);
            $passnameEsc = $dbcon->real_escape_string($passname);
            $strInsert="INSERT INTO users (nameuser, passname, role) VALUES ('".$nameuserEsc."','".$passnameEsc."',".$role.")";
                        ...
                        <table class="w-full border-collapse" id="dataTables-example">
                            ...
                            while($qryData=$qryRes->fetch_assoc()){
                                ...
                                <tr class="hover:bg-gray-50 transition duration-150">
                                    ...
                                    <td class="px-4 py-3 text-gray-600"><?php echo htmlspecialchars($passname, ENT_QUOTES, 'UTF-8');?></td>
                ...
                <form role="form" method="post" action="<?php echo $formActionUrl; ?>">
                    ...
                    <div class="modal-body p-6 text-left">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="passname" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="Password" required>
                        </div>
                        <div class="mb-4">
                    ...
                    <div class="modal-footer bg-gray-50 px-6 py-4 flex justify-end gap-3">
                        ...
                        <button type="submit" name="btnAdd" class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg shadow-md transition duration-200"><i class="icon-save"></i> Save User</button>
                    </div>
                </form>
            </div>
        </div>
                ...
                <form method="post" role="form" action="<?php echo $formActionUrl; ?>" class="w-full">
                    ...
                    <div class="modal-body p-6 text-left">
                        <div class="mb-4">
                            ...
                            <input type="text" name="unameuser" id="edit_username" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" required>
                        </div>
                        <div class="mb-4">
                            <label class="block text-gray-700 font-semibold mb-2">User Password</label>                                                             
                            <input type="text" name="upassname" id="edit_password" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" required>
                        </div>
                        <div class="mb-4">
                            <label class="block text-gray-700 font-semibold mb-2">User Role</label>  
                            <select name="urole" id="edit_role" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">
                                <?php
                                $roleQry="SELECT rid, rname FROM role";
                                $roleRes=$dbcon->query($roleQry);
                                while($r=$roleRes->fetch_assoc()){
                                    echo "<option value='".$r['rid']."'>".htmlspecialchars($r['rname'], ENT_QUOTES, 'UTF-8')."</option>";
                                }
                                ?>
                            </select>
                        </div>
            ...
            <form method="post" action="<?php echo $formActionUrl; ?>" role="form" class="w-full">
                <div class="modal-content rounded-lg shadow-2xl">
                    <div class="bg-red-600 px-6 py-4 flex justify-between items-center">
                        <h4 class="text-lg font-bold text-white"><i class="icon-trash"></i> Remove System User Account</h4>
